    ...
    <>
      <header className="sticky top-0 z-40 bg-white/95 backdrop-blur-md shadow-sm border-b border-slate-100 font-sans">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 flex items-center justify-between gap-4">
          ...
          {/* Right Action Section - Single Clean User Button */}
          <div className="flex items-center gap-3">
            ...
            {/* SINGLE CLEAN USER BUTTON */}
            {isAuthenticated ? (
              <button
                onClick={() => setProfileModalOpen(true)}
                className="px-4 py-2 rounded-full bg-emerald-50 text-[#00A651] hover:bg-emerald-100 text-xs font-extrabold border border-emerald-200 flex items-center gap-1.5 transition-all shadow-xs cursor-pointer"
              >
                <UserCheck className="w-4 h-4" />
                <span>My Account ({user?.name || 'Farmer'})</span>
              </button>
            ) : (
              <button
                onClick={() => setRegisterModalOpen(true)}
                className="flex items-center gap-1.5 px-4.5 py-2.5 rounded-full bg-[#00A651] text-white hover:bg-[#008440] text-xs font-extrabold transition-all shadow-sm cursor-pointer"
              >
